...
class __AuxBoxGymObsSpace:
    ...
    def __init__(
        ...
        self._dict_properties = {
            ...
            "gen_p": (
                np.full(shape=(len(mask['gen_p']),), fill_value=0.0, dtype=dt_float)
                - tol_redisp
                - extra_for_losses,
                ob_sp_cls.gen_pmax[mask['gen_p']] + tol_redisp + extra_for_losses,
                (len(mask['gen_p']),),
                dt_float,
            ),
            "gen_q": (
                np.full(shape=(len(mask['gen_p']),), fill_value=-np.inf, dtype=dt_float),
                np.full(shape=(len(mask['gen_p']),), fill_value=np.inf, dtype=dt_float),
                (len(mask['gen_p']),),
                dt_float,
            ),
            ...
            "gen_margin_up": (
                np.full(shape=(len(mask['gen_p']),), fill_value=0.0, dtype=dt_float),
                1.0 * ob_sp_cls.gen_max_ramp_up[mask['gen_p']],
                (len(mask['gen_p']),),
                dt_float,
            ),
            "gen_margin_down": (
                np.full(shape=(len(mask['gen_p']),), fill_value=0.0, dtype=dt_float),
                1.0 * ob_sp_cls.gen_max_ramp_down[mask['gen_p']],
                (len(mask['gen_p']),),
                dt_float,
            ),
            "gen_theta": (
            ...
            "load_p": (
                np.full(shape=(len(mask['load_p']),), fill_value=-np.inf, dtype=dt_float),
                np.full(shape=(len(mask['load_p']),), fill_value=+np.inf, dtype=dt_float),
                (len(mask['load_p']),),
                dt_float,
            ),
            ...
            "load_v": (
                np.full(shape=(len(mask['load_p']),), fill_value=0.0, dtype=dt_float),
                np.full(shape=(len(mask['load_p']),), fill_value=np.inf, dtype=dt_float),
                (len(mask['load_p']),),
                dt_float,
            ),
            "load_theta": (
                np.full(shape=(len(mask['load_p']),), fill_value=-180., dtype=dt_float),
                np.full(shape=(len(mask['load_p']),), fill_value=180., dtype=dt_float),
                (len(mask['load_p']),),
                dt_float,
            ),
            "p_or": (
                np.full(shape=(len(mask['p_or']),), fill_value=-np.inf, dtype=dt_float),
                np.full(shape=(len(mask['p_or']),), fill_value=np.inf, dtype=dt_float),
                (len(mask['p_or']),),
                dt_float,
            ),
            ...
            "a_or": (
                np.full(shape=(len(mask['p_or']),), fill_value=0.0, dtype=dt_float),
                np.full(shape=(len(mask['p_or']),), fill_value=np.inf, dtype=dt_float),
                (len(mask['p_or']),),
                dt_float,
            ),
            ...
            "theta_or": (
                np.full(shape=(len(mask['p_or']),), fill_value=-180., dtype=dt_float),
                np.full(shape=(len(mask['p_or']),), fill_value=180., dtype=dt_float),
                (len(mask['p_or']),),
                dt_float,
            ),
            ...
            "rho": (
                np.full(shape=(len(mask['rho']),), fill_value=0.0, dtype=dt_float),
                np.full(shape=(len(mask['rho']),), fill_value=np.inf, dtype=dt_float),
                (len(mask['rho']),),
                dt_float,
            ),
            "line_status": (
                np.full(shape=(len(mask['line_status']),), fill_value=0, dtype=dt_int),
                np.full(shape=(len(mask['line_status']),), fill_value=1, dtype=dt_int),
                (len(mask['line_status']),),
                dt_int,
            ),
            "timestep_overflow": (
                np.full(
                    shape=(len(mask['timestep_overflow']),), fill_value=np.iinfo(dt_int).min, dtype=dt_int
                ),
                np.full(
                    shape=(len(mask['timestep_overflow']),), fill_value=np.iinfo(dt_int).max, dtype=dt_int
                ),
                (len(mask['timestep_overflow']),),
                dt_int,
            ),
            "topo_vect": (
                np.full(shape=(len(mask['topo_vect']),), fill_value=-1, dtype=dt_int),
                np.full(shape=(len(mask['topo_vect']),), fill_value=ob_sp_cls.n_busbar_per_sub, dtype=dt_int),
                (len(mask['topo_vect']),),
                dt_int,
            ),
            "time_before_cooldown_line": (
                np.full(shape=(len(mask['rho']),), fill_value=0, dtype=dt_int),
                np.full(
                    shape=(len(mask['rho']),), fill_value=np.iinfo(dt_int).max, dtype=dt_int
                ),
                (len(mask['rho']),),
                dt_int,
            ),
            "time_before_cooldown_sub": (
                np.full(shape=(len(mask['time_before_cooldown_sub']),), fill_value=0, dtype=dt_int),
                np.full(
                    shape=(len(mask['time_before_cooldown_sub']),), fill_value=np.iinfo(dt_int).max, dtype=dt_int
                ),
                (len(mask['time_before_cooldown_sub']),),
                dt_int,
            ),
            "time_next_maintenance": (
                np.full(shape=(len(mask['rho']),), fill_value=-1, dtype=dt_int),
                np.full(
                    shape=(len(mask['rho']),), fill_value=np.iinfo(dt_int).max, dtype=dt_int
                ),
                (len(mask['rho']),),
            ...
            "target_dispatch": (
                np.minimum(ob_sp_cls.gen_pmin[mask['gen_p']], -ob_sp_cls.gen_pmax[mask['gen_p']]),
                np.maximum(-ob_sp_cls.gen_pmin[mask['gen_p']], +ob_sp_cls.gen_pmax[mask['gen_p']]),
                (len(mask['gen_p']),),
                dt_float,
            ),
            "actual_dispatch": (
            ...
            "curtailment": (
                np.full(shape=(len(mask['curtailment']),), fill_value=0.0, dtype=dt_float),
                np.full(shape=(len(mask['curtailment']),), fill_value=1.0, dtype=dt_float),
                (len(mask['curtailment']),),
                dt_float,
            ),
            "curtailment_limit": (
                np.full(shape=(len(mask['curtailment_limit']),), fill_value=0.0, dtype=dt_float),
                np.full(shape=(len(mask['curtailment_limit']),), fill_value=1.0, dtype=dt_float),
                (len(mask['curtailment_limit']),),
                dt_float,
            ),
            "curtailment_mw": (
                np.full(shape=(len(mask['curtailment']),), fill_value=0.0, dtype=dt_float),
                1.0 * ob_sp_cls.gen_pmax[mask['curtailment']],
                (len(mask['curtailment']),),
                dt_float,
            ),
    ...
    def to_gym(self, grid2op_observation):
        ...
        for attr_nm, where_to_put in zip(self._attr_to_keep, self._dims):
            ...
            if len(self.mask[attr_nm]) > 1:
                res[prev:where_to_put] = tmp[self.mask[attr_nm]]
            else: 
...
if GYMNASIUM_AVAILABLE:
    ...
    MaskedBoxGymObsSpace = BoxGymnasiumObsSpace
    MaskedBoxGymObsSpace.__doc__ = __AuxBoxGymObsSpace.__doc__
